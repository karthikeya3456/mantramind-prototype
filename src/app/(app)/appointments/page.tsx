...
export type SessionType = 'Virtual Consultation' | 'In-Person Meeting';

export default function AppointmentsPage() {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedSession, setSelectedSession] = useState<SessionType | null>(null);

  const handleBookClick = (sessionType: SessionType) => {
      ...
      <div className="space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card className="flex flex-col">
            ...
            <CardFooter>
              <Button className="w-full" onClick={() => handleBookClick('Virtual Consultation')}>
                <CalendarIcon className="mr-2 h-4 w-4" /> Book Virtual Session
              </Button>
            </CardFooter>
          </Card>
          ...
          <Card className="flex flex-col">
            ...
            <CardFooter>
              <Button className="w-full" onClick={() => handleBookClick('In-Person Meeting')}>
                  <CalendarIcon className="mr-2 h-4 w-4" /> Book In-Person Session
              </Button>
            </CardFooter>
          </Card>
        </div>
